...
function ActiveSpeakerView({
  ...
  const whiteboardPlugin = useRealtimeKitSelector((m) =>
    m.plugins.active.get(WHITEBOARD_ID)
  );
  ...
  const canSpotlight = useRealtimeKitSelector(
    (m) => m.self.permissions.canSpotlight
  );
    ...
    if (screenshares.length > screensharesRef.current.length) {
      ...
      if (canSpotlight) {
        meeting.meta.setSelfActiveTab(
          {
            type: 'screenshare',
            id: newScreenshare.id,
          },
          0
        );
      }
      setSelectedTab({ type: 'screenshare', participant: newScreenshare });
    }
    screensharesRef.current = screenshares;
  }, [screenshares, canSpotlight, meeting.meta]);
  ...
  useEffect(() => {
    if (plugins.length > pluginsRef.current.length) {
      ...
      setSelectedTab({ type: 'plugin', plugin: newPlugin });
    }
    pluginsRef.current = plugins;
  }, [plugins, canSpotlight, meeting.meta]);

  useEffect(() => {
    if (selectedTab) return;
    onFallback();
  }, [selectedTab, onFallback]);
  ...
  }, [screenshares, plugins, selectedTab, onFallback]);
  ...
  const setConfig = () => {
    ...
    const hostId = isHost
      ...
      : meeting.participants.joined
          .toArray()
          .find((x) => x.presetName === HOST_PRESET)?.id;
      ...
      {plugins.map((plugin) => (
        ...
        >
          ...
          >
            ...
            >
              <RtkIcon
                icon={iconPack.dismiss}
                size="sm"
                onClick={() => closePlugin(plugin)}
              />
            </div>
          </div>
...
export default function MainArea() {
  ...
  const isActiveView = useMemo(
    () => screenshares.length + activePlugins.length > 0,
    [screenshares.length, activePlugins.length]
  );
          ...
          <div className="center">
            {isHost && (
              <div className="flex flex-col items-center">
                ...
                <span className="text-text-md my-4">You are off stage.</span>
              </div>
            )}
            {!isHost && (
              <div className="flex flex-col items-center">
